...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const pathname = h.get("x-pathname") || "/world-clock";
...
// JSON-LD Structured Data for World Clock page
function WorldClockJsonLd({ lang }: { lang: Lang }) {
  const structuredData = {
    ...
    mainEntity: {
      ...
      featureList: [
        "Live world clock for thousands of cities",
...
// FAQ JSON-LD (from your SEO content)
function FaqJsonLd({ lang }: { lang: Lang }) {
  const seoContent = translations[lang].seo?.worldclock;
...
export default async function WorldClockLayout({ children }: { children: ReactNode }) {
  const h = await headers();
  const pathname = h.get("x-pathname") || "/world-clock";
  const lang = getLangFromPath(pathname);
