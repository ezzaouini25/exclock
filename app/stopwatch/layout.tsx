...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const pathname = h.get("x-pathname") || "/stopwatch";
...
// JSON-LD Structured Data for Stopwatch page
function StopwatchJsonLd({ lang }: { lang: Lang }) {
...
function FaqJsonLd({ lang }: { lang: Lang }) {
  const seoContent = translations[lang].seo?.stopwatch;
...
export default async function StopwatchLayout({ children }: { children: ReactNode }) {
  const h = await headers();
  const pathname = h.get("x-pathname") || "/stopwatch";
  const lang = getLangFromPath(pathname);
